...
<form action="<%=request.getContextPath()%>/board/donBoardUpdate.do" method="post" enctype="multipart/form-data">
	<input type="hidden" name="board_no" value="3">
	<input type="hidden" name="mem_no" value="<%=memberVo.getMem_no()%>">
제목:	<input type="text" name="post_title" class="form-control mt-4 mb-2"
		placeholder="제목을 입력해주세요." value="<%=request.getAttribute("post_title")%>" quired
	>
내용:<div class="form-group">
		<textarea class="form-control" rows="10" name="post_cont"
			placeholder="내용을 입력해주세요" required><%=request.getAttribute("post_cont")%></textarea>
	</div>
 파일 업로드:<input type="file" name="file"><br>
 <%if(request.getAttribute("do_video")!=null){ %>
 <input type="hidden" name="do_video" value="<%=request.getAttribute("do_video")%>">
  <% }%>
	<input type="hidden" name="post_no" value="<%=request.getAttribute("post_no")%>">
